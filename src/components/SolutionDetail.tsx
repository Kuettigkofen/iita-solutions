...
interface SolutionDetailProps {
  solution: Solution;
  onShare: (type: 'pdf' | 'webpdf' | 'email' | 'contact' | 'faq') => void;
  userRole?: string;
}
...
export function SolutionDetail({ solution, onShare, userRole }: SolutionDetailProps) {
  ...
  const getTitleImage = () => {
    console.log('Solution images:', solution.solution_images);
    const titleImage = solution.solution_images?.find(img => img.image_type === 'title_image');
    console.log('Title image found:', titleImage);
    return titleImage?.image_url || solution.image || 'https://images.unsplash.com/photo-1516974080664-ed2fc6a32937?w=600&h=400&fit=crop&crop=center';
  };
  ...
  const getProblemImage = () => {
    ...
    return problemImage?.image_url || 'https://images.unsplash.com/photo-1464207687429-7505649dae38?w=600&h=400&fit=crop&crop=center';
  };
  ...
  const getSolutionImage = () => {
    ...
    return solutionImage?.image_url || 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=600&h=400&fit=crop&crop=center';
  };
  ...
  const getSolutionBulletpoints = () => {
    ...
    return bulletpoints.map((point, index) => {
      // Split by first sentence ending (. followed by space and any character)
      const sentences = point.split(/\.(?=\s)/);
      const title = sentences[0] + '.'; // First sentence as title
  ...
  // Convert markdown links to HTML strings
  const convertMarkdownToHTML = (text: string) => {
    if (!text) return '';

    // First handle HTML links generated by Word parser (already HTML)
    if (text.includes('<a href=')) {
      return text.replace(
        /<a href="([^"]*)"[^>]*>([^<]*)<\/a>/g,
        '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-orange-500 hover:text-orange-400 underline transition-colors">$2</a>'
      );
    }

    // Handle markdown-style links [text](url)
    return text.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-orange-500 hover:text-orange-400 underline transition-colors">$1</a>');
  };
  ...
  const getExecutiveSummaryWithRole = () => {
    ...
    // Convert markdown links to HTML in base summary
    let processedSummary = convertMarkdownToHTML(baseSummary);

    if (roleSpecificSummary && processedSummary.includes('[Role-specific relevance placeholder.]')) {
      const highlightedSummary = `<span class="bg-orange-500/20 text-orange-100 px-2 py-0.5 rounded">${roleSpecificSummary}</span>`;
      return processedSummary.replace('[Role-specific relevance placeholder.]', highlightedSummary);
    }

    // If no placeholder found but we have role-specific content, append it with highlighting
    if (roleSpecificSummary && processedSummary) {
      const highlightedSummary = `<span class="bg-orange-500/20 text-orange-100 px-2 py-0.5 rounded">${roleSpecificSummary}</span>`;
      return `${processedSummary} ${highlightedSummary}`;
    }
  ...
  // Parse markdown links and HTML hyperlinks
  const parseMarkdownLinks = (text: string) => {
    if (!text) return [];

    // First handle HTML links generated by Word parser
    if (text.includes('<a href=')) {
      return (
        <span
