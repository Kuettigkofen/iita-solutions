...
// Read batch 4 data
console.log('\nReading batch4_population_results.json...');
const newData = JSON.parse(fs.readFileSync('batch4_population_results.json', 'utf8'));
...
newData.forEach(newPub => {
  ...
  if (matchingRow) {
    ...
    if (matchingRow.DOI && matchingRow.Abstract && matchingRow.Keywords) {
      alreadyComplete++;
      console.log(`  ✓ Already complete [Row ${newPub.rowNumber}]`);
    } else if (!newPub.DOI || newPub.DOI === "" || newPub.DOI === "Not available") {
      // Skip if no DOI found
      skipped++;
      console.log(`  ⊘ Skipped [Row ${newPub.rowNumber}]: No DOI available`);
    } else {
      ...
      updatedCount++;
      console.log(`  ✓ Updated [Row ${newPub.rowNumber}]: ${newPub.status}`);
