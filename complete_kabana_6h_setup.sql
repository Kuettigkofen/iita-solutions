-- Complete KABANA 6H Setup: Schema Updates + Data Insertion
-- Run this entire file in your Supabase SQL Editor

-- Step 1: Add summary sentence columns for each role
ALTER TABLE solutions
ADD COLUMN IF NOT EXISTS funder_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS policymaker_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS farmer_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS student_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS extensionofficer_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS researcher_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS devpractitioner_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS businessowner_summarysentence TEXT,
ADD COLUMN IF NOT EXISTS businessowner_text TEXT;

-- Step 2: Update data types for climate_potential and key_agroeco
ALTER TABLE solutions
...
-- Step 3: Create images table for solution visuals
...
-- Step 4: Add external references field if not exists
ALTER TABLE solutions
ADD COLUMN IF NOT EXISTS external_references TEXT[];

-- Step 5: Create indexes for better performance (if not exists)
CREATE INDEX IF NOT EXISTS idx_solution_images_solution_id ON solution_images (solution_id);
CREATE INDEX IF NOT EXISTS idx_solution_images_type ON solution_images (image_type);

-- Step 6: Enable Row Level Security for images table
ALTER TABLE solution_images ENABLE ROW LEVEL SECURITY;

-- Step 7: Create policy for public read access on images
...
-- Step 8: Create trigger for updated_at timestamp on images
DROP TRIGGER IF EXISTS update_solution_images_updated_at ON solution_images;
CREATE TRIGGER update_solution_images_updated_at
    BEFORE UPDATE ON solution_images
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Step 9: Insert KABANA 6H solution data
...
-- Step 10: Display the inserted solution ID for image creation
SELECT
  id as solution_id,
  solution_title,
  'Copy this UUID for image entries' as note
FROM solutions
WHERE solution_title = 'KABANA 6H/NARITA7 Hybrid: High-Yielding and Disease-Tolerant Banana';